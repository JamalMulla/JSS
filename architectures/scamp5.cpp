...
void SCAMP5::pushToNews(AREG& src, AREG& dst, news_t dir) {
    int width = src.value().cols;
    int height = src.value().rows;
    switch (dir) {
        case east: {
            auto chunk = cv::Rect(1, 0, width - 1, height);
            src.value()(cv::Rect(0, 0, width - 1, height)).copyTo(dst.value()(chunk), FLAG.value()(chunk));
            dst.value()(cv::Rect(0, 0, 1, height)).setTo(cv::Scalar(-1), FLAG.value()(cv::Rect(0, 0, 1, height)));
            break;
        }
        case west: {
            auto chunk = cv::Rect(0, 0, width - 1, height);
            src.value()(cv::Rect(1, 0, width - 1, height)).copyTo(dst.value()(chunk), FLAG.value()(chunk));
            dst.value()(cv::Rect(width - 1, 0, 1, height)).setTo(cv::Scalar(-1), FLAG.value()(cv::Rect(width - 1, 0, 1, height)));
            break;
        }
        case north: {
            auto chunk = cv::Rect(0, 0, width, height - 1);
            src.value()(cv::Rect(0, 1, width, height - 1)).copyTo(dst.value()(chunk), FLAG.value()(chunk));
            dst.value()(cv::Rect(0, height - 1, width, 1)).setTo(cv::Scalar(-1), FLAG.value()(cv::Rect(0, height - 1, width,1)));
            break;
        }
        case south: {
            auto chunk = cv::Rect(0, 1, width, height - 1);
            src.value()(cv::Rect(0, 0, width, height - 1)).copyTo(dst.value()(chunk), FLAG.value()(chunk));
            dst.value()(cv::Rect(0, 0, width, 1)).setTo(cv::Scalar(-1), FLAG.value()(cv::Rect(0, 0, width, 1)));
            break;
        }
        default:
            break;
...
void SCAMP5::pullFromNews(AREG& dst, AREG& src, news_t dir) {
    int width = src.value().cols;
    int height = src.value().rows;
    switch (dir) {
        case west: {
            auto chunk = cv::Rect(0, 0, width - 1, height);
            auto n = src.value()(cv::Rect(1, 0, width - 1, height));
            n.copyTo(dst.value()(chunk), FLAG.value()(chunk));
            dst.value()(cv::Rect(width - 1, 0, 1, height)).setTo(cv::Scalar(-1), FLAG.value()(cv::Rect(width - 1, 0, 1, height)));
            break;
        }
        case east: {
...
//=======
//    switch (dir) {
//        case north: this->pe.analogue_bus.push_south(x0, NEWS, 2, FLAG); break;
//        case east: this->pe.analogue_bus.push_west(x0, NEWS, 2, FLAG); break;
//        case west: this->pe.analogue_bus.push_east(x0, NEWS, 2, FLAG); break;
//        case south: this->pe.analogue_bus.push_north(x0, NEWS, 2, FLAG); break;
//        case alldir: std::cerr << "Unhandled direction" << std::endl; break;
//    }
//    this->bus(y, NEWS);
    AREG intermediate(y.value().rows, y.value().cols);
    this->bus(intermediate, x0);
    this->pushToNews(intermediate, NEWS, dir);
    this->bus(y, NEWS);
}
...
//    }
//    switch (dir2) {
//        case north: this->pe.analogue_bus.push_north(NEWS, y, 2, FLAG); break;
//        case east: this->pe.analogue_bus.push_east(NEWS, y, 2, FLAG); break;
//        case west: this->pe.analogue_bus.push_west(NEWS, y, 2, FLAG); break;
//        case south: this->pe.analogue_bus.push_south(NEWS, y, 2, FLAG); break;
//        case alldir: std::cerr << "Unhandled direction" << std::endl; break;
//    }
    AREG intermediate(y.value().rows, y.value().cols);
    ...
    this->bus(intermediate, x0);
    this->pushToNews(intermediate, NEWS, dir);
    this->pullFromNews(intermediate2, NEWS, dir2);
...
//    switch (dir) {
//        case north: this->pe.analogue_bus.push_south(intermediate, NEWS, 2, FLAG); break;
//        case east: this->pe.analogue_bus.push_west(intermediate, NEWS, 2, FLAG); break;
//        case west: this->pe.analogue_bus.push_east(intermediate, NEWS, 2, FLAG); break;
//        case south: this->pe.analogue_bus.push_north(intermediate, NEWS, 2, FLAG); break;
//        case alldir: std::cerr << "Unhandled direction" << std::endl; break;
//    }
//    this->bus(y, NEWS);
//    switch (dir) {
