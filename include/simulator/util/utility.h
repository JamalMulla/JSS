...
void display_register(const std::string& window_name, Register& reg) {
    ...
    cv::setMouseCallback(window_name, utility::on_mouse_reg<interpret_type>,
                         &reg);
...
void on_mouse_reg(int event, int x, int y, int, void* reg) {
    ...
    if(y < 0 || x < 0 || y > dr->read().rows || x > dr->read().cols)
        return;
    std::cout << "(" << x << ", " << y << ") ......  "
              << (int) dr->read().getMat(cv::ACCESS_READ).at<interpret_type>(y, x) << '\n';
}
...
void display_mat(const std::string& window_name, cv::Mat& src) {
    ...
    cv::namedWindow(window_name, cv::WINDOW_NORMAL);
    cv::resizeWindow(window_name, 256, 256);
    cv::setMouseCallback(window_name, utility::on_mouse_mat<interpret_type>,
                         &src);
    cv::imshow(window_name, draw);
}
...
void on_mouse_mat(int event, int x, int y, int, void* src) {
    ...
    if(y < 0 || x < 0 || y > dr->rows || x > dr->cols)
        return;
    std::cout << "(" << x << ", " << y << ") ......  "
              << (int)dr->at<interpret_type>(y, x) << '\n';
}
