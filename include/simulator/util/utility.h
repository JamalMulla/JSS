...
namespace utility {
    ...
    template <typename interpret_type>
    void display_mat(const std::string& window_name, cv::UMat& src);
    template <typename interpret_type>
    void on_mouse_mat(int event, int x, int y, int, void* src);
    void remap_register(Register& reg, cv::UMat& dst);
    void remap_mat(cv::UMat& src, cv::UMat& dst);
    ...
    void on_mouse_reg(int event, int x, int y, int, void* reg) {
        ...
        std::cout<<"("<<x<<", "<<y<<") ......  "<< (int) dr->value().getMat(cv::ACCESS_READ).at<interpret_type>(y,x) <<'\n';
    }
    ...
    void on_mouse_mat(int event, int x, int y, int, void *src) {
        if ( event != cv::EVENT_MOUSEMOVE ) return;
        auto* dr = static_cast<cv::UMat*>(src);
        if (y < 0 || x < 0 || y > dr->rows || x > dr->cols) return;
        std::cout<<"("<<x<<", "<<y<<") ......  "<< (int) dr->getMat(cv::ACCESS_READ).at<interpret_type>(y,x) <<'\n';
    }
