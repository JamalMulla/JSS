...
SCENARIO("analogue bus correctly manipulates analogue registers when mask=all") {
    ...
    GIVEN("some number of analogue registers and a mask") {
        ...
        AnalogueRegister a = (cv::Mat)(cv::Mat_<int>(rows, cols) << 1, 2, 3, 4, 5, 6, 7, 8, 9);
        ...
        WHEN("two registers dst and a") {
            THEN("dst = -a") {
                bus.bus(dst, a, mask);
                cv::Mat expected = (cv::Mat)(cv::Mat_<int>(rows, cols) << -1, -2, -3, -4, -5, -6, -7, -8, -9);
                REQUIRE(utility::are_mats_equal(dst.value(), expected));
            }
        }

        WHEN("3 registers, dst, a and b") {
            AnalogueRegister b = (cv::Mat)(cv::Mat_<int>(rows, cols) << 1, 2, 3, 4, 5, 6, 7, 8, 9);
            THEN("dst = -(a + b)") {
                bus.bus(dst, a, b, mask);
                cv::Mat expected = (cv::Mat)(cv::Mat_<int>(rows, cols) << -2, -4, -6, -8, -10, -12, -14, -16, -18);
                REQUIRE(utility::are_mats_equal(dst.value(), expected));
            }
        }
    }
